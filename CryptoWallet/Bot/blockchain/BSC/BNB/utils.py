...
async def check_address_for_transfer(address):
    ...
    try:
        await get_balance_in_BNB(address)
        return True
    except:
        return False
...
# Получение баланса BNB aSync
async def get_balance_in_BNB(address):
    bsc_node = "https://bsc-dataseed1.binance.org:443"
    web3 = AsyncWeb3(AsyncWeb3.AsyncHTTPProvider(bsc_node))
    balance = await web3.eth.get_balance(address) #получаем баланс кошелька
    return balance/10**18  #возвращаем баланс в BNB


#Получить курс BNB USDT async
async def get_BNB_in_USDT():
    try:
        url = 'https://api.binance.com/api/v3/ticker/price?symbol=BNBUSDT' #ссылка для запроса
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                data = await response.json()
                bnb_to_usd = float(data['price'])
                return bnb_to_usd #возвращаем значение цены 
    except Exception as e:
        print(f"Произошла ошибка при получении курса BNB в USDT: {e}")

#Получить курс USD RUB async
async def get_USD_in_RUB():
    try:
        url = 'https://api.binance.com/api/v3/ticker/price?symbol=USDTRUB'#ссылка для запроса
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                data = await response.json()
                bnb_to_rub = float(data['price'])
                return bnb_to_rub #возвращаем значение цены 
    except Exception as e:
        print(f"Произошла ошибка при получении курса BNB в USDT: {e}")
