...
from Bot.blockchain.BSC.BTC import utils
...
async def send_to_bnb_menu(bot: Bot, chat_id: int, balance_token: float, USDinRUB: float, message_id=None):
    ...
    TOKENinUSD = balance_token * await utils.get_BTC_in_USDT()
    TOKENinRUB = TOKENinUSD * float(USDinRUB)
    url = "https://bscscan.com/token/0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c?a=" + adress

    bnb_menu = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Отправить BTC", callback_data="transfer_BTC_BSC_plane")],
...
@router.callback_query(lambda query: query.data.startswith('BTC_BSC:'))
