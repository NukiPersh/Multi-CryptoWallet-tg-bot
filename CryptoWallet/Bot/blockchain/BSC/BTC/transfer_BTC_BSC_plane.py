...
from Bot.blockchain.BSC.BTC import utils, send_transaction
...
class Transfer_BTC_BSC(StatesGroup):
    address = State()
    sum = State()
    wait = State()
    password = State()
...
@router.callback_query(lambda query: query.data == 'transfer_BTC_BSC_plane')
async def transfer_menu_callback(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot,):
    if (await database_func.check_wallet_in_db(callback_query.message.chat.id)) == True:
        await state.set_state(Transfer_BTC_BSC.address)
        await callback_query.answer()
        await state.update_data(bot_message_id=callback_query.message.message_id)
        txt = "Введите адресс получателя BTC (СЕТЬ BSC):"
...
@router.message(Transfer_BTC_BSC.address)
async def fill_address_transfer(message: Message, state: FSMContext, bot: Bot):
    ...
    if await utils.check_address_for_transfer(addr) == True: #условие что это адрес
        ...
        await state.set_state(Transfer_BTC_BSC.sum)
        await message.delete()
        maxs = await balance.get_token_balance(await database_func.get_address(message.chat.id), btctoken)
        max_sum = await asyncio.to_thread(lambda: max(0, maxs))
        txt = f"Введите сумму в BTC:\nМаксимум: {max_sum}"
...
@router.callback_query(lambda query: query.data == 'change_sum_bsc_btc')
async def change_sum_transfer(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot,):
    maxs = (await balance.get_token_balance(await database_func.get_address(callback_query.message.chat.id), btctoken))
    max_sum = await asyncio.to_thread(lambda: max(0, maxs)) 
    txt = f"Введите сумму в BTC:\nМаксимум: {max_sum}"
    await bot.edit_message_text(chat_id=callback_query.message.chat.id,
                                message_id=callback_query.message.message_id,
...
@router.message(Transfer_BTC_BSC.sum)
async def fill_sum_transfer(message: Message, state: FSMContext, bot:Bot):
    ...
    if float(summ) > 0 and summ.isdigit and (float(summ) <= (await balance.get_token_balance(await database_func.get_address(message.chat.id), btctoken))):#проверка что это число и что денег хватит на балансе
        txt = f"<b>Информация о транзакции:</b>\n\nПолучатель:\n{data_from_state.get('address')}\n\nСумма: {summ} BTC"
        await state.update_data(sum=summ)
        await state.set_state(Transfer_BTC_BSC.wait)
        await message.delete()
        await bot.edit_message_text(chat_id=message.chat.id,
                                    message_id=bot_message_id,
                                    text=txt,
                                    reply_markup=transfer_check_btc
                                    ) 
    else:
        await message.delete()
        txt = f"Не верная сумма! Вы не можете отправить {summ} BTC"
...
@router.callback_query(lambda query: query.data == 'accept_transfer_bsc_btc')
async def accept_transfer(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot,):
    await state.set_state(Transfer_BTC_BSC.password)
...
@router.message(Transfer_BTC_BSC.password)
async def password_accept_transfer(message: Message, state: FSMContext, bot:Bot):
    ...
    if len(passw) > 3 and await utils.check_password_for_pk(message.chat.id, passw):#проверка что это пароль
        ...
        tx = await send_transaction.send_token_btc(await cryptography_pkeys.decrypt_private_key(await database_func.get_privacy_key(message.chat.id), passw),
...
transfer_check_btc = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(text="Подтвердить", callback_data="accept_transfer_bsc_btc")
        ],
        [
            InlineKeyboardButton(text="Изменить сумму", callback_data="change_sum_bsc_btc")
        ],
    ],
    resize_keyboard=True,  # для адаптации размера кнопок
    one_time_keyboard=True,  # скрытие после использования
