...
async def check_address_for_transfer(address):
    ...
    try:
        await get_balance_in_BTC(address)
        return True
    except:
        return False
...
# Получение баланса BTC aSync
async def get_balance_in_BTC(address):
    bsc_node = "https://bsc-dataseed1.binance.org:443"
    web3 = AsyncWeb3(AsyncWeb3.AsyncHTTPProvider(bsc_node))
    balance = await web3.eth.get_balance(address) #получаем баланс кошелька
    return balance/10**18  #возвращаем баланс в BTC


#Получить курс BTC USDT async
async def get_BTC_in_USDT():
    try:
        url = 'https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT' #ссылка для запроса
        ...
        print(f"Произошла ошибка при получении курса BTC в USDT: {e}")
