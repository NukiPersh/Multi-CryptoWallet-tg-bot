...
from Bot.blockchain.BSC.ETH import utils
...
async def send_to_bnb_menu(bot: Bot, chat_id: int, balance_token: float, USDinRUB: float, message_id=None):
    ...
    TOKENinUSD = balance_token * await utils.get_ETH_in_USDT()
    TOKENinRUB = TOKENinUSD * float(USDinRUB)
    url = "https://bscscan.com/token/0x2170Ed0880ac9A755fd29B2688956BD959F933F8?a=" + adress

    bnb_menu = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Отправить ETH", callback_data="transfer_ETH_BSC_plane")],
...
@router.callback_query(lambda query: query.data.startswith('ETH_BSC:'))
