...
async def check_address_for_transfer(address):
    ...
    try:
        await get_balance_in_ETH(address)
        return True
    except:
        return False
...
async def get_balance_in_ETH(address):
    ...
    return balance/10**18  #возвращаем баланс в ETH


#Получить курс BNB USDT async
async def get_ETH_in_USDT():
    try:
        url = 'https://api.binance.com/api/v3/ticker/price?symbol=ETHUSDT' #ссылка для запроса
        ...
        print(f"Произошла ошибка при получении курса ETH в USDT: {e}")
