...
from Bot.blockchain.BSC.LTC import utils
...
async def send_to_bnb_menu(bot: Bot, chat_id: int, balance_token: float, USDinRUB: float, message_id=None):
    ...
    TOKENinUSD = balance_token * await utils.get_LTC_in_USDT()
    TOKENinRUB = TOKENinUSD * float(USDinRUB)
    url = "https://bscscan.com/token/0x4338665CBB7B2485A8855A139b75D5e34AB0DB94?a=" + adress

    bnb_menu = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Отправить LTC", callback_data="transfer_LTC_BSC_plane")],
...
@router.callback_query(lambda query: query.data.startswith('LTC_BSC:'))
