...
from Bot.blockchain.BSC.SOL import utils
...
async def send_to_bnb_menu(bot: Bot, chat_id: int, balance_token: float, USDinRUB: float, message_id=None):
    ...
    TOKENinUSD = balance_token * await utils.get_SOL_in_USDT()
    TOKENinRUB = TOKENinUSD * float(USDinRUB)
    url = "https://bscscan.com/token/0x570A5D26f7765Ecb712C0924E4De545B89fD43dF?a=" + adress

    bnb_menu = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Отправить SOL", callback_data="transfer_SOL_BSC_plane")],
...
@router.callback_query(lambda query: query.data.startswith('SOL_BSC:'))
