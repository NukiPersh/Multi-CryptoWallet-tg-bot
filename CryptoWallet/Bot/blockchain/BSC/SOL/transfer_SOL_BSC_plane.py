...
async def fill_address_transfer(message: Message, state: FSMContext, bot: Bot):
    ...
    if await utils.check_address_for_transfer(addr) == True: #условие что это адрес
        ...
        maxs = (await balance.get_balance_in_SOL(await database_func.get_address(message.chat.id)) - await utils.get_gas_price() * 1.1)
...
async def change_sum_transfer(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot,):
    maxs = (await balance.get_balance_in_SOL(await database_func.get_address(callback_query.message.chat.id)) - await utils.get_gas_price() * 1.1)
    max_sum = await asyncio.to_thread(lambda: max(0, maxs)) 
    txt = f"Введите сумму в SOL:\nМаксимум: {max_sum}"
    await bot.edit_message_text(chat_id=callback_query.message.chat.id,
                                message_id=callback_query.message.message_id,
...
async def fill_sum_transfer(message: Message, state: FSMContext, bot:Bot):
    ...
    if float(summ) > 0 and summ.isdigit and (float(summ) <= (await balance.get_balance_in_SOL(await database_func.get_address(message.chat.id)) - await utils.get_gas_price() * 1.1)):#проверка что это число и что денег хватит на балансе
        summ = message.text
...
async def password_accept_transfer(message: Message, state: FSMContext, bot:Bot):
    ...
    if len(passw) > 3 and await utils.check_password_for_pk(message.chat.id, passw):#проверка что это пароль
        ...
        tx = await send_transaction.send_token(await cryptography_pkeys.decrypt_private_key(await database_func.get_privacy_key(message.chat.id), passw),
                                  ...
                                  amount_in_sol=float(await asyncio.to_thread(data_from_state.get, 'sum'))
                                  )
