...
async def check_address_for_transfer(address):
    ...
    try:
        await get_balance_in_TON(address)
        return True
    except:
        return False
...
# Получение баланса TON aSync
async def get_balance_in_TON(address):
    bsc_node = "https://bsc-dataseed1.binance.org:443"
    web3 = AsyncWeb3(AsyncWeb3.AsyncHTTPProvider(bsc_node))
    balance = await web3.eth.get_balance(address) #получаем баланс кошелька
    return balance/10**18  #возвращаем баланс в TON


#Получить курс TON USDT async
async def get_TON_in_USDT():
    try:
        url = 'https://api.binance.com/api/v3/ticker/price?symbol=TONUSDT' #ссылка для запроса
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                data = await response.json()
                ton_to_usd = float(data['price'])
                return ton_to_usd #возвращаем значение цены 
    except Exception as e:
        print(f"Произошла ошибка при получении курса TON в USDT: {e}")
