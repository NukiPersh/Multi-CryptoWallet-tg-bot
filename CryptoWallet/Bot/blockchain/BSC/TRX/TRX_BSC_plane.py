...
from Bot.blockchain.BSC.TRX import utils
...
async def send_to_bnb_menu(bot: Bot, chat_id: int, balance_token: float, USDinRUB: float, message_id=None):
    ...
    TOKENinUSD = balance_token * await utils.get_TRX_in_USDT()
    TOKENinRUB = TOKENinUSD * float(USDinRUB)
    url = "https://bscscan.com/token/0xCE7de646e7208a4Ef112cb6ed5038FA6cC6b12e3?a=" + adress


    bnb_menu = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Отправить TRX", callback_data="transfer_TRX_BSC_plane")],
...
@router.callback_query(lambda query: query.data.startswith('TRX_BSC:'))
