...
from Bot.blockchain.BSC.USDC import utils
...
async def send_to_bnb_menu(bot: Bot, chat_id: int, balance_token: float, USDinRUB: float, message_id=None):
    ...
    TOKENinUSD = balance_token * await utils.get_USDC_in_USDT()
    TOKENinRUB = TOKENinUSD * float(USDinRUB)
    url = "https://bscscan.com/token/0x8ac76a51cc950d9822d68b83fe1ad97b32cd580d?a=" + adress

    bnb_menu = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Отправить USDC", callback_data="transfer_USDC_BSC_plane")],
...
@router.callback_query(lambda query: query.data.startswith('USDC_BSC:'))
