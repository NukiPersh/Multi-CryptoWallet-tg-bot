...
from Bot.blockchain.BSC.USDC import utils, send_transaction 
from Bot.blockchain.BSC import balance
from Cryptography import cryptography_pkeys

router = Router()

class Transfer_usdc_BSC(StatesGroup):  
    address = State()
    sum = State()
    wait = State()
    password = State()
...
@router.callback_query(lambda query: query.data == 'transfer_USDC_BSC_plane')  
async def transfer_menu_callback(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot):
    if (await database_func.check_wallet_in_db(callback_query.message.chat.id)) == True:
        await state.set_state(Transfer_usdc_BSC.address)
        await callback_query.answer()
        await state.update_data(bot_message_id=callback_query.message.message_id)
        txt = "Введите адрес получателя usdc (СЕТЬ BSC):"  
...
@router.message(Transfer_usdc_BSC.address)
async def fill_address_transfer(message: Message, state: FSMContext, bot: Bot):
    ...
    if await utils.check_address_for_transfer(addr) == True:
        ...
        await state.set_state(Transfer_usdc_BSC.sum)
        await message.delete()
        maxs = await balance.get_token_balance(await database_func.get_address(message.chat.id), usdctoken)
        max_sum = await asyncio.to_thread(lambda: max(0, maxs))
        txt = f"Введите сумму в usdc:\nМаксимум: {max_sum}" 
...
@router.callback_query(lambda query: query.data == 'change_sum_bsc_usdc')  
async def change_sum_transfer(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot):
    maxs = (await balance.get_token_balance(await database_func.get_address(callback_query.message.chat.id), usdctoken))
    max_sum = await asyncio.to_thread(lambda: max(0, maxs)) 
    txt = f"Введите сумму в USDC:\nМаксимум: {max_sum}"  
    await bot.edit_message_text(chat_id=callback_query.message.chat.id,
                              message_id=callback_query.message.message_id,
...
@router.message(Transfer_usdc_BSC.sum)
async def fill_sum_transfer(message: Message, state: FSMContext, bot:Bot):
    ...
    if float(summ) > 0 and summ.isdigit and (float(summ) <= (await balance.get_token_balance(await database_func.get_address(message.chat.id), usdctoken))):
        txt = f"<b>Информация о транзакции:</b>\n\nПолучатель:\n{data_from_state.get('address')}\n\nСумма: {summ} USDC"  
        await state.update_data(sum=summ)
        await state.set_state(Transfer_usdc_BSC.wait)
        await message.delete()
        await bot.edit_message_text(chat_id=message.chat.id,
                                  message_id=bot_message_id,
                                  text=txt,
                                  reply_markup=transfer_check_usdc  
                                  ) 
    else:
        await message.delete()
        txt = f"Неверная сумма! Вы не можете отправить {summ} USDC"  
...
@router.callback_query(lambda query: query.data == 'accept_transfer_bsc_usdc')  
async def accept_transfer(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot):
    await state.set_state(Transfer_usdc_BSC.password)
...
@router.message(Transfer_usdc_BSC.password)
async def password_accept_transfer(message: Message, state: FSMContext, bot:Bot):
    ...
    if len(passw) > 3 and await utils.check_password_for_pk(message.chat.id, passw):
        ...
        data_from_state = await state.get_data()
        tx = await send_transaction.send_token_usdc( 
...
transfer_check_usdc = InlineKeyboardMarkup(  
    inline_keyboard=[
        [
            InlineKeyboardButton(text="Подтвердить", callback_data="accept_transfer_bsc_usdc")  
        ],
        [
            InlineKeyboardButton(text="Изменить сумму", callback_data="change_sum_bsc_usdc")  
        ],
    ],
    resize_keyboard=True,
    one_time_keyboard=True,
